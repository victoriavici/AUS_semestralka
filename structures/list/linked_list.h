...
namespace structures
{
    ...
    protected:
        ...
        LinkedListItem<T>* createItem(T Data);
        void connect(LinkedListItem<T>* item1, LinkedListItem<T>* item2);
    ...
    {
        auto* newLLI = createItem(data);
        if (size_ == 0) {
            first_ = newLLI;
        }
        else {
            this->connect(last_, newLLI);
        }
        last_ = newLLI;
        size_++;
    }
    ...
    inline void LinkedList<T>::insert(const T& data, int index)
    {
        if (index == size_) {
            ...
            auto* newLLI = createItem(data);
            if (index == 0) {
                this->connect(newLLI, first_);
                first_ = newLLI;
            }
            else {
                auto pred = getItemAtIndex(index - 1);
                this->connect(newLLI, pred->getNext());
                this->connect(pred, newLLI);
            }
            size_++;
        }
    }
    ...
    inline T LinkedList<T>::removeAt(int index)
    {
        ...
        if (index == 0) {
            ...
            this->connect(pred, delItem->getNext());
            if (last_ == delItem) {
                last_ = pred;
            }
        }
    ...
    template<typename T>
    inline LinkedListItem<T>* LinkedList<T>::createItem(T Data)
    {
        return new LinkedListItem<T>(Data);
    }

    template<typename T>
    inline void LinkedList<T>::connect(LinkedListItem<T>* item1, LinkedListItem<T>* item2)
    {
        item1->setNext(item2);
    }
