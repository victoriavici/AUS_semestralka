...
namespace structures
{
	...
	inline void BinarySearchTree<K, T>::extractNode(BSTTreeNode* node)
	{
		if (!node->hasLeftSon() && !node->hasRightSon()) {
			...
			auto inorder = node->getRightSon();
			while (inorder->hasLeftSon()) {
				inorder = inorder->getLeftSon();
			}
			extractNode(inorder);
			if (node->isRoot()) {
				binaryTree_->replaceRoot(inorder);
			}
			inorder->setParent(node->getParent());
			inorder->changeRightSon(node->getRightSon());
			inorder->changeLeftSon(node->getLeftSon());
